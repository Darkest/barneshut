...
package object barneshut {
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body])
    ...
    val massX = bodies.foldLeft(0f)((acc, el) => acc + el.x * el.mass) / mass
    val massY = bodies.foldLeft(0f)((acc, el) => acc + el.y * el.mass) / mass

    val total: Int = bodies.length

    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        val newfork = Fork(Empty(centerX - size / 4, centerY - size / 4, size / 2),
          Empty(centerX + size / 4, centerY - size / 4, size / 2),
          Empty(centerX - size / 4, centerY + size / 4, size / 2),
          Empty(centerX + size / 4, centerY + size / 4, size / 2))

        (b +: bodies).foldLeft(newfork)((fork, body) => fork.insert(body))
      }
